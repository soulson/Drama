...
namespace Drama.Host.Providers
{
	...
	/// </remarks>
	public class FileStorage : BaseJsonStorageProvider
	{
		/// <summary>
	...
	internal class GrainStateFileDataManager : IJsonStateDataManager
	{
		...
		{
			this.directory = new DirectoryInfo(storageDirectory);
		...
		{
			FileInfo fileInfo = this.GetStorageFilePath(collectionName, key);
		...
		private FileInfo GetStorageFilePath(string collectionName, string key)
		{
			...
			string path = Path.Combine(this.directory.FullName, fileName);
